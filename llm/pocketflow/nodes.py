...
from .utils import call_llm, log_llm_message
import yaml


class NoticePrompt(Node):
    def prep(self, shared):
        main_prompt = shared["main_prompt"]
        prompt = f""" {main_prompt}
            Ничего пока не делай, ожидай моих инструкций.
        """
        shared["messages"].append({"role": "system", "content": prompt})
        log_llm_message(
            run=shared["chain_run"],
            message_type="user",
            message_text=prompt
        )
    ...
    def post(self, shared, prep_res,exec_res):
        ...
        log_llm_message(
            ...
            message_text=exec_res
        )


class SetInputs(Node):
    def prep(self, shared):
        ...
        prompt = f"""
           ...
           Ничего пока не делай, ожидай моих инструкций.
        """
        shared["messages"].append({"role": "user", "content": prompt})
...
class MakeStep(Node):
    def prep(self, shared):
        step_num =  shared["current_step"]
        prompt = f""" Выполни шаг {step_num}"""
        shared["messages"].append({"role": "user", "content": prompt})
        log_llm_message(
            run=shared["chain_run"],
            message_type="user",
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        if "```json" in exec_res:
            try:
                json_str = exec_res.split("```json")[1].split("```")[0].strip()
                structured_result = json.loads(json_str)
            except (IndexError, json.JSONDecodeError) as e:
                raise ValueError(f"Ошибка при обработке JSON на шаге {shared['current_step']}: {e}")
        else:
            structured_result = exec_res
        shared["results"].append(structured_result)
